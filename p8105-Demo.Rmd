...
most_reordered<-instacart_fresh_vegetabe %>% filter(reordered ==1 ) %>% 
  count(product_name)%>%arrange(desc(n))%>%head(20)%>%
...
## The distribution of days since prior order for the 20 most reordered items from the fresh item aisle
```{r}

ten_most_popular<-most_reordered%>%pull(`product name`)

instacart_fresh_vegetabe %>% filter(product_name %in% as.character(ten_most_popular)[1:20] )%>%
